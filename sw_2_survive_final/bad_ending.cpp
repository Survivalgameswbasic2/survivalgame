...
void bad_ending_zombie() {
    ...
    updateTextBox("당신은 좀비가 되어 사망했습니다");
...
static std::vector<std::vector<std::string>> bad_ending__time_dialogue = {
    "시간이 너무 늦어 좀비들이 늘어났다",
    "좀비가 너무 많아 감당하지 못하였다"
    "Bad Ending"
};
...
void bad_ending_time() {
    ...
    while (1) {
        if (_kbhit()) {
            int key = _getch();
            if (key == ' ') { // 스페이스바로 대화 진행
                if (dialogue_line < bad_ending__time_dialogue[dialogue_num].size()) {
                    updateDialogue(bad_ending__time_dialogue[dialogue_num][dialogue_line++]);
                }
                else {
                    // 대화 종료
                    exit(0);
