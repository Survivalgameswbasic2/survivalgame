...
/////////////////////////////////////////////////////////////////////////
//햇빛 정보 업데이트
void update_info_sun(int remainingTime) {
	//*로 바꾸기
	if (remainingTime >=150) //왼
		for (int i = 1; i < STAGE_HEIGHT; i++) {
			for (int j = 1; j < STAGE_WIDTH; j++) {
				if (game_map[i][j] == '#')break;
				else game_map[i][j] = '*';
			}
		}
	else {//위
		...
		for (int i = 1; i < STAGE_WIDTH; i++) {
			for (int j = 1; j < STAGE_HEIGHT; j++) {
				if (game_map[j][i] == '#')break;
				else game_map[j][i] = '*';
			}
		}
	}
	...
	for (int y = 0; y < STAGE_HEIGHT; y++) {
		for (int x = 0; x < STAGE_WIDTH; x++) {
			setCursorPosition(x * 2 + 1, y + 1); // 두 칸씩 차지하도록 x * 2 + 1
			if (game_map[y][x] == '#'); //벽은 그대로 유지
			else if (game_map[y][x] == '*') {	
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), BACKGROUND_RED | BACKGROUND_GREEN); //노란색 출력
				printf(" ");
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); //검은색으로 리셋
			}
		}
	}
}
...
void sun_reduce_health(int playerX, int playerY) {
	//체력깎기
	while (1) {
		if (game_map[playerY][playerX] == '*')
			std::this_thread::sleep_for(std::chrono::seconds(1)); //1초 이후에
		if (game_map[playerY][playerX] != '*')return;			//그늘에 있으면 종료
		if (game_map[playerY][playerX] == '*') health--;		//햇빛에 있으면 health 감소
	}
}
...
int main() {
	...
	std::thread healthThread(sun_reduce_health);
