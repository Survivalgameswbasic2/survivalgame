...
void drawStage2() {
	...
	for (int y = 0; y < STAGE_HEIGHT; y++) {
		for (int x = 0; x < STAGE_WIDTH; x++) {
			...
			case 'P': setColor(10); printf("■"); setColor(7); break; //플레이어
...
void drawZombie() {
	for (int y = 0; y < STAGE_HEIGHT; y++) {
		for (int x = 0; x < STAGE_WIDTH; x++) {
			...
			case 'N': break;
			case 'P': break;
			case 'x': setColor(12); printf("■"); setColor(7); break; // 좀비 출력
			default: printf("  "); break;
			}
		}
	}
}
void zombieMoveThread(std::vector<Zombie>& zombies, char game_map[STAGE_HEIGHT][STAGE_WIDTH+1]) {
	while (true) {
		// 1초마다 좀비를 이동
...
void start_day0(player* user) {
	...
	//좀비 이동 별도 쓰레드
	std::thread zombieThread(zombieMoveThread, std::ref(zombies), game_map);
	game_map[user->player_y][user->player_x] = 'P';
